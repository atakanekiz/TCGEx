  ...
  moduleServer(id,function(input,output,session) {
    ...
    cvfit <- eventReactive(c(input$run_train, input$run_traintest), {
      ...
      if (input$regression_workflow == "create_model") {
        ...
        fitty <- cv.glmnet(predictor_var, response_var,weights = NULL, foldid = foldid, alpha = input$user_alpha, family="binomial")
      } else if (input$regression_workflow == "ctest_model") {
        ...
        fitty <- cv.glmnet(predictor_var_train, response_var_train, foldid = foldid, alpha = input$user_alpha, family="binomial")
      } 
      fitty
    })
